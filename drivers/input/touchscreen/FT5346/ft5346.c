...
static void Gesture_Fucntion_Proc_File(void);
...
static const char gtp_gesture_type[]=GTP_GESTURE_TPYE_STR;
...
static void check_gesture(int gesture_id, struct input_dev *ip_dev)
{
	...
	switch(gesture_id)
	{
		case 0x24:
			gtp_gesture_value = 'K';
			input_report_key(ip_dev, KEY_GESTURE, 1);
			input_sync(ip_dev);
			input_report_key(ip_dev, KEY_GESTURE, 0);
			input_sync(ip_dev);
			break;
		case 0x21:
			gtp_gesture_value = 'R';
		...
		case 0x30:
			gtp_gesture_value = 'o';
//begin-fix by hubing1.wt for gesture -2016.10.12
			gtp_gesture_coordinate[13] = 'o';
			input_report_key(ip_dev, KEY_GESTURE, 1);
			input_sync(ip_dev);
			input_report_key(ip_dev, KEY_GESTURE, 0);
			input_sync(ip_dev);
		...
		case 0x46:
			gtp_gesture_value = 's';
			gtp_gesture_coordinate[13] = 's';
			input_report_key(ip_dev, KEY_GESTURE, 1);
			input_sync(ip_dev);
			input_report_key(ip_dev, KEY_GESTURE, 0);
			input_sync(ip_dev);
			break;
//begin-add by hubing1.wt for gesture -2016.10.13
		case 0x53:
			gtp_gesture_value = '^';
			...
			input_sync(ip_dev);
			break;
//end-add by hubing1.wt for gesture -2016.10.13
		case 0x54:
			gtp_gesture_value = 'v';
			gtp_gesture_coordinate[13] = 'v';
			input_report_key(ip_dev, KEY_GESTURE, 1);
			input_sync(ip_dev);
			input_report_key(ip_dev, KEY_GESTURE, 0);
			input_sync(ip_dev);
		...
		case 0x65:
		      gtp_gesture_value = 'z';
			gtp_gesture_coordinate[13] = 'z';
//end-fix by hubing1.wt for gesture -2016.10.12
			input_report_key(ip_dev, KEY_GESTURE, 1);
			input_sync(ip_dev);
			input_report_key(ip_dev, KEY_GESTURE, 0);
			input_sync(ip_dev);
...
//end-fix by hubing1.wt for gesture -2016.10.12
	...
	if (fts_updateinfo_curr.CHIP_ID==0x54|| fts_updateinfo_curr.CHIP_ID==0x58)
	{
		...
		{
//begin-fix by hubing1.wt for gesture -2016.10.12
			coordinate_x[i] =  (((s16) buf[2 + (4 * i)]) & 0xFF) <<
				8 | (((s16) buf[3 + (4 * i)])& 0xFF);
			coordinate_y[i] = (((s16) buf[4 + (4 * i)]) & 0xFF) <<
				8 | (((s16) buf[5 + (4 * i)]) & 0xFF);
			//printk("%d----0x%04x,0x%04x\n",i,coordinate_x[i],coordinate_y[i]);
		...
		//begin-point
		gtp_gesture_coordinate[15] = coordinate_x[0]& 0xFF;
		gtp_gesture_coordinate[16] = (coordinate_x[0]>>8)& 0xFF;
		gtp_gesture_coordinate[17] = coordinate_y[0]& 0xFF;
		gtp_gesture_coordinate[18] = (coordinate_y[0]>>8)&0xFF;
		//end-point
		gtp_gesture_coordinate[19] = coordinate_x[pointnum-1]& 0xFF;
		gtp_gesture_coordinate[20] = (coordinate_x[pointnum-1]>>8)&0XFF;
		gtp_gesture_coordinate[21] = coordinate_y[pointnum-1]& 0xFF;
		gtp_gesture_coordinate[22] = (coordinate_y[pointnum-1]>>8)&0XFF;
		//width/heigh
		gtp_gesture_coordinate[23] = (x_max-x_min)& 0xFF;
		gtp_gesture_coordinate[24] = ((x_max-x_min)>>8)&0XFF;
		gtp_gesture_coordinate[25] = (y_max-y_min)& 0xFF;
		gtp_gesture_coordinate[26] = ((y_max-y_min)>>8)&0XFF;
		//center-point
		gtp_gesture_coordinate[27] = ((x_max+x_min)/2)& 0xFF;
		gtp_gesture_coordinate[28] = (((x_max+x_min)/2)>>8)&0XFF;
		gtp_gesture_coordinate[29] = ((y_max+y_min)/2)& 0xFF;
		gtp_gesture_coordinate[30] = (((y_max+y_min)/2)>>8)&0XFF;
		//four special point
		gtp_gesture_coordinate[31] = gtp_gesture_coordinate[15] ;
		gtp_gesture_coordinate[32] = gtp_gesture_coordinate[16] ;
		gtp_gesture_coordinate[33] = gtp_gesture_coordinate[17] ;
		gtp_gesture_coordinate[34] = gtp_gesture_coordinate[18] ;

		gtp_gesture_coordinate[35] = coordinate_x[pointnum/4]& 0xFF;
		gtp_gesture_coordinate[36] = (coordinate_x[pointnum/4]>>8)& 0xFF;
		gtp_gesture_coordinate[37] = coordinate_y[pointnum/4]& 0xFF;
		gtp_gesture_coordinate[38] = (coordinate_y[pointnum/4]>>8)& 0xFF;

		gtp_gesture_coordinate[39] = coordinate_x[pointnum*3/4]& 0xFF;
		gtp_gesture_coordinate[40] = (coordinate_x[pointnum*3/4]>>8)& 0xFF;
		gtp_gesture_coordinate[41] = coordinate_y[pointnum*3/4]& 0xFF;
		gtp_gesture_coordinate[42] = (coordinate_y[pointnum*3/4]>>8)& 0xFF;

		gtp_gesture_coordinate[43] = gtp_gesture_coordinate[19] ;
		gtp_gesture_coordinate[44] = gtp_gesture_coordinate[20] ;
		gtp_gesture_coordinate[45] = gtp_gesture_coordinate[21] ;
		gtp_gesture_coordinate[46] = gtp_gesture_coordinate[22] ;
...
//Modify, wangdongbo.wt, 20160520_pinctl
...
static unsigned char CTPM_FW1[]= //oufei+tianma+white
{
#include "ft_app_ic_oufei.txt"
};
/*
static unsigned char CTPM_FW2[]= //oufei+tianma+black
{
#include "ft_app_ic_oufei_black.txt"
};
static unsigned char CTPM_FW3[]= //oufei+tianma+golden
{
#include "ft_app_ic_oufei_golden.txt"
};
*/
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	for (i = 0; i < FT_UPGRADE_LOOP; i++)
	{
		...
		ft5x06_i2c_write(client, &auc_i2c_write_buf[0], 1);
		msleep(5);
		...
		/* check READ_ID */
		msleep(fts_updateinfo_curr.delay_readid);
		auc_i2c_write_buf[0] = FT_READ_ID_REG;
		...
		ft5x06_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		 ...
		 CTP_DEBUG("reg_val[0] :%X,reg_val[1]: %X\n", reg_val[0], reg_val[1]);
	       ...
	       CTP_DEBUG("[FTS] OK: FTS_UPGRADE_LOOP is  i = %d \n", i);
		auc_i2c_write_buf[0] = 0xcd;
		ft5x06_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
		CTP_DEBUG("[FTS]bootloader version = 0x%x\n", reg_val[0]);
		...
		for(i = 0;i < FT_UPGRADE_LOOP; i++)
		{
			...
			if (i_ret < 0)
			{
				CTP_DEBUG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
				continue;
			}
			//fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);

			i_ret = ft5x06_i2c_read(client, auc_i2c_write_buf, 0, reg_val, 2);
			if (i_ret < 0)
			{
				CTP_DEBUG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
				continue;
			...
			vid = reg_val[1];
			CTP_DEBUG("%s: REG VAL ID1 = 0x%x,ID2 = 0x%x\n", __func__,reg_val[0],reg_val[1]);
			break;
		}
		CTP_DEBUG("Step 7: reset the new FW\n");
...
//use to check gesture function
fts_updateinfo_curr.CHIP_ID = ic_type;

    CTP_DEBUG("Vendor ID:0x%02X, TP FW:0x%02X, IC TYPE:%d", vendor_id,uc_tp_fm_ver,ic_type);

    if((ic_type != CTP_IC_TYPE_0) && (ic_type != CTP_IC_TYPE_1) && (ic_type != CTP_IC_TYPE_2))
    {
        CTP_ERROR("IC type dismatch, please check");
    }

    if(vendor_id == 0xA8 || vendor_id == 0x00 || vendor_id == 0xEF ||ic_type == 0xA3 || ic_type == 0x00 || ic_type == 0xEF )
    {
    ...
    if(vendor_id == 0x51)//OUFEI + TIANMA
    {
        pbt_buf = CTPM_FW1;
    ...
    if (sizeof(CTPM_FW1)< 8 || sizeof(CTPM_FW1) > 64 * 1024 )
    {
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
    ...
    *ppos += count;
    ...
    {
        sprintf(cfgname, "%s", "ft5346_oufei.ini");
...
{
	CTP_DEBUG("in proc_gesture_onoff_write \n");
    sscanf(buffer,"%c",&gtp_gesture_onoff);
    gtp_gesture_support_flag_changed=1;
	CTP_DEBUG("in proc_gesture_onoff_write %c \n", gtp_gesture_onoff);
    return count;
}
...
void Gesture_Fucntion_Proc_File(void)
{
    struct proc_dir_entry *ctp_device_proc = NULL;
...
#define CTP_GESTURE_FUNCTION_AUTHORITY_PROC 0777
    ...
    {
        CTP_DEBUG("ctp_gesture_type_proc create failed\n");
    }
    ...
    {
        CTP_DEBUG("create_proc_entry version failed\n");
    }
...
//fix by hubing1.wt for change irq way _2016.10.21
